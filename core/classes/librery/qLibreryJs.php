...
class qLibreryJs {
     ...
     * @var array
     */
    protected $options = ['flaggedComments' => true];
    ...
    public function minify($js) {
        ...
        if ($this->error) {
            qLog::write($this->error);
            return false;
        }
        $js = ltrim($this->result);
        $js = $this->unlock($js);
        return $js;
    ...
    protected function processMultiLineComments($startIndex)
    {
        ...
        if ($this->getNext('*/')) {
            ...
            if (($this->options['flaggedComments'] && $thirdCommentString === '!')
                ...
                $this->result .= substr($this->input, $startIndex, $endPoint);
        ...
        if($char === false) {
            $this->error = 'Unclosed multiline comment at position: ' . ($this->index - 2);
        }
    ...
    protected function saveString()
    {
        ...
        while (true) {
            ...
            switch ($this->a) {
                ...
                // block below.
                case "\n":
                    $this->error = 'Unclosed string at position: ' . $startpos;
                    break;
    ...
    protected function saveRegex()
    {
        ...
        while (($this->a = $this->getChar()) !== false) {
            ...
            if($this->a === "\n") {
                $this->error = 'Unclosed regex pattern at position: ' . $this->index;
            }
            $this->result .=  $this->a;
        }
        $this->b = $this->getReal();
